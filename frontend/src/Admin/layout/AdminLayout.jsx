...
import { fetchRecentCvs, checkForNewCvs, markAsRead, markAllAsRead, addCvUploadNotification, calculateUnreadCount, fetchNotifications, getUnreadCount, markNotificationAsRead, markAllNotificationsAsReadAsync, deleteNotification, deleteAllNotifications } from '../../features/notifications/notificationSlice'
...
const AdminLayout = () => {
  ...
  const { newCvNotifications, unreadCount, lastChecked, notifications } = useSelector((state) => state.notifications)
  ...
  // Fetch notifications on login and poll for updates
  useEffect(() => {
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6">
                  {(newCvNotifications.length > 0 || notifications.length > 0) ? (
                    <div className="space-y-4">
                      {/* Display Socket.IO notifications */}
                      {newCvNotifications.map((notification) => (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 cursor-pointer" onClick={() => dispatch(markAsRead(notification.id))}>
                      ...
                      {notifications.map((notification) => (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <div className={`w-2 h-2 rounded-full mt-2 ${
                              notification.isReadByAdmin ? 'bg-slate-400' : 'bg-blue-600'
                            }`}></div>
                            <div className="flex-1 cursor-pointer" onClick={() => dispatch(markNotificationAsRead(notification._id))}>
                              <p className="font-medium text-slate-900 dark:text-white">
                                {notification.title || 'New CV Upload'}
                              </p>
                              <p className="text-sm text-slate-600 dark:text-slate-400">
                                {notification.message}
                              </p>
