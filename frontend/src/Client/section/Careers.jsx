...
const Careers = ({ setCvData, jobPostings, isLoggedIn, setIsLoggedIn }) => {
  ...
  const [selectedCountryCode, setSelectedCountryCode] = useState('+353') // Default to Ireland
  ...
  const validatePhone = (phone) => {
    // Very simple validation - just check if phone exists and has reasonable length
    // The react-phone-input-2 component handles the actual phone validation
  ...
  const handlePhoneChange = (value, isValid, countryCode) => {
    setCareerForm(c => ({ ...c, phone: value || '' }));
    
    // Use the country code passed from the PhoneInput component
    if (countryCode) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validatePhone(careerForm.phone)) {
      setCareerStatus('Please enter a phone number.');
      return;
    }
    ...
    try {
      ...
      // Use the tracked country code and clean phone number
      const phoneNumber = careerForm.phone || '';
      const cleanPhone = phoneNumber.replace(/[\s\-\(\)]/g, '');
      
      // Remove country code from phone number since we're storing it separately
      const phoneWithoutCountryCode = cleanPhone.replace(selectedCountryCode, '');
      
      // Add contact information using the selected country code
            ...
            <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full h-12 rounded-lg bg-background border px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-ring transition-colors ${
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full h-12 rounded-lg bg-background border border-input px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-ring focus:border-primary transition-colors"
                    >
                ...
                <button
                  ...
                  className="w-full h-12 rounded-lg bg-primary px-5 py-3 text-base font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-60 shadow-md transition-all duration-300"
                >
