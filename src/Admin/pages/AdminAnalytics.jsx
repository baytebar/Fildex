...
const AdminAnalytics = () => {
  ...
  const newCvs = cvData.filter(cv => cv.status === 'new').length
  const reviewedCvs = cvData.filter(cv => cv.status === 'reviewed').length
  const shortlistedCvs = cvData.filter(cv => cv.status === 'shortlisted').length
  const rejectedCvs = cvData.filter(cv => cv.status === 'rejected').length
  ...
  const avgApplications = totalJobs > 0 ? 
    Math.round(jobPostings.reduce((sum, job) => sum + job.applicants, 0) / totalJobs) : 0

  // Get recent activity (last 7 days)
  const recentCvs = cvData.filter(cv => {
