...
const AdminRoleManagement = () => {
  ...
  const { cvData = [], setCvData = () => {}, availableRoles = [] } = context || {}
  const [activeTab, setActiveTab] = useState('users')
  const [searchTerm, setSearchTerm] = useState('')
  const [showRoleDialog, setShowRoleDialog] = useState(false)
  const [selectedUser, setSelectedUser] = useState(null)

  // Create users list from CV data with additional user management fields
  const users = useMemo(() => {
    if (!cvData || !Array.isArray(cvData)) return []
    
    return cvData.filter(cv => cv && cv.name && cv.email && cv.role).map(cv => ({
      ...cv,
      lastLogin: '2024-01-15', // Mock data
      loginCount: Math.floor(Math.random() * 50) + 1,
      permissions: cv.userRole && availableRoles ? availableRoles.find(role => role.id === cv.userRole)?.permissions || [] : []
    }))
  }, [cvData, availableRoles])
  ...
  const assignRole = useCallback((userId, roleId) => {
    setCvData(prev => prev.map(cv => 
      cv.id === userId ? { ...cv, userRole: roleId } : cv
    ))
    closeRoleDialog()
  }, [setCvData, closeRoleDialog])
